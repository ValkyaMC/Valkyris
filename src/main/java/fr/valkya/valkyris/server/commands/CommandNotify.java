...
public class CommandNotify extends CommandBase {

	@Override
	public String getCommandName() { return "notify"; }
	...
	public void processCommand(ICommandSender sender, String[] args) {
		if(args.length >= 1) {
		    ...
		    for(EntityPlayer player : PlayerUtils.getAllPlayers()) {
				Valkyris.getValkyris().getProxy().getNetwork().sendTo(new PacketNotify(EnumChatFormatting.RED + "Notification du Serveur", msg, NotificationType.INFO, 5), (EntityPlayerMP) player);
			}
		    sender.addChatMessage(new ChatComponentText(EnumChatFormatting.GREEN + "Message envoyé!"));
		}else {
			sender.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "La commande est : /notify <message>"));
		}
	}
}
