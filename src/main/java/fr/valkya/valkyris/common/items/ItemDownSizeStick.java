...
public class ItemDownSizeStick extends ValoStickItem {
	
	public ItemDownSizeStick(String name, CreativeTabs tab, int maxStackSize) {
	...
	public void doEffect(ItemStack it, World w, EntityPlayer player) {
		if(!w.isRemote) {
			((ServerProxy)Valkyris.getValkyris().getProxy()).getSizeManager().add((EntityPlayerMP)player, EntitySize.SMALL, 15);
		}
	}
}
