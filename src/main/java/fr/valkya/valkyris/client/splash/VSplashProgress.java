package fr.valkya.valkyris.client.splash;
...
public class VSplashProgress 
{
	private static Animation animation = new Animation(0, 0);
	...
	public static void start()
    {
        ...
        final ResourceLocation logoLoc = new ResourceLocation("fml:textures/gui/title-full.png");
        final ResourceLocation bgLoc = new ResourceLocation("fml:textures/gui/background.png");
...
//                        }
//                    }
                	ProgressBar first = ProgressManager.barIterator().hasNext() ? ProgressManager.barIterator().next() : null;
                    ...
                    glPushMatrix();
                    glOrtho(0.0D, (double)w, (double)h, 0.0D, -1.0D, 1.0D);
                    glMatrixMode(5888);
                    glLoadIdentity();
                    ...
                    bgTexture.texCoord(0, 0.0F, 0.0F);
                    glVertex2f(0.0F, 0.0F);

                    bgTexture.texCoord(0, 0.0F, 1.0F);
                    glVertex2f(0.0F, (float)h);

                    bgTexture.texCoord(0, 1.0F, 1.0F);
                    glVertex2f((float)w, (float)h);

                    bgTexture.texCoord(0, 1.0F, 0.0F);
                    ...
                    if(first != null)
                    {
                        ...
                        glScalef(2, 2, 1);
                        glEnable(GL_TEXTURE_2D);
                        fontRenderer.drawString(first.getTitle() + " - " + first.getMessage(), 0, 0, 0x000000);
                        glDisable(GL_TEXTURE_2D);
                        glPopMatrix();
                        ...
                        animation.interpolate((barWidth - 2) * (first.getStep() + 1) / (first.getSteps() + 1), 0, 2.5); // Step can sometimes be 0.
                        
                        drawBox((int) animation.getX(), barHeight - 2); 
                        
                        // progress text
                        String progress = "" + first.getStep() + "/" + first.getSteps();
    ...
    private static class Texture
    {
        ...
        public Texture(ResourceLocation location)
        {
            ...
            try
            {
                ...
                this.size = size;
                glEnable(GL_TEXTURE_2D);
                synchronized(VSplashProgress.class)
                {
                    name = glGenTextures();
